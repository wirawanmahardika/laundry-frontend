...
const StatusBadge = ({ status }: { status: PesananForm['status'] }) => {
  ...
  const badgeClass = status === "selesai" ? "badge-success" : 
                    status === "proses" ? "badge-warning" : "badge-info";
  
  return (
    <span className={`badge ${badgeClass} badge-md text-center`}>
      {statusText}
    </span>
  );
};
...
export default function EditPesanan() {
  ...
  const handleAddLayanan = () => {
    ...
    setPesanan(prev => ({
      ...prev,
      layanans: [
        ...prev.layanans.filter(l => l.id !== layananBaru.id),
        layananBaru
      ]
    }));
  ...
  <div className="w-full mt-6">
    ...
    {showDetail && (
      <div className="flex flex-col gap-y-3 mt-2">
        ...
        <ServiceListSection 
          layanans={pesanan.layanans}
          onDeleteLayanan={onDeleteLayanan}
        />
