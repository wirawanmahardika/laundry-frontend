    ...
    const tambahPesanan = async (e: any) => {
        ...
        if (layanans.length === 0)
            return Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Pesanan wajib memiliki setidaknya satu layanan',
            });
        const body = {
        ...
        try {
            const res = await AxiosAuth.post('/transaksi', body, {
            ...
            <form className="bg-base-100 rounded-xl shadow-lg p-8 flex flex-col items-center gap-y-4 w-full max-w-lg border border-base-300">
                ...
                <div className="flex flex-col gap-y-2 w-full">
                    ...
                    <div className="flex gap-x-2">
                        <select
                            onChange={(e) => {
                                ...
                                if (selectedLayanan)
                                    setLayanan((p) => ({
                                        ...p,
                                        id: selectedLayanan.id,
                                        name: selectedLayanan.nama,
                        ...
                        <input
                            onChange={(e) =>
                                setLayanan((p) => ({ ...p, quantity: parseInt(e.target.value) }))
                            }
                            type="number"
                            min={1}
                    ...
                    <button
                        ...
                        onClick={() => {
                            if (layanan.name && layanan.quantity > 0) {
                                if (layanans.find((l) => l.id === layanan.id))
                                    dispatch({ type: 'replace', payload: layanan });
                ...
                <button
                    ...
                    className="btn btn-primary btn-md mt-3 w-full shadow"
                >
                    Tambah Pesanan
                </button>
            </form>
