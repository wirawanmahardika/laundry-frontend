...
export default function Profil() {
    ...
    useEffect(() => {
        AxiosAuth.get('/tenant').then((res) => setProfil(res.data.data));
    }, []);
        ...
        <div className="container mx-auto max-w-2xl px-4 py-8 flex flex-col gap-y-6 items-center">
            ...
            <div className="bg-base-100 rounded-xl p-5 flex flex-col gap-y-3 w-full shadow-lg border border-base-200">
                <div className="flex justify-between items-center mb-2">
                    ...
                    <NavLink
                        to={'/profil/edit'}
                        className="btn btn-accent btn-xs rounded-full px-4 shadow"
                    >
                        Edit Profile
                    </NavLink>
                </div>
            ...
            <div className="bg-base-100 rounded-xl p-5 flex flex-col gap-y-3 w-full shadow border border-base-200">
                <div className="flex items-center gap-x-2 mb-2">
                    ...
                    <h2 className="text-base sm:text-lg font-semibold text-sky-700">
                        Subscription Info
                    </h2>
                </div>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-y-2">
                    <div>
                        ...
                        {dayjs().isBefore(profil?.masa_berlaku) ? (
                    ...
                    <div>
                        ...
                        <span className="ml-2 font-semibold text-slate-700">
                            {dayjs(profil?.masa_berlaku).format('D MMM YYYY')}
                        </span>
                    </div>
                </div>
                <div className="flex gap-x-2 mt-2">
                    <button className="btn btn-outline btn-xs btn-info rounded-full">
                        Upgrade
                    </button>
                    <button className="btn btn-outline btn-xs btn-error rounded-full">
                        Cancel
                    </button>
                </div>
            </div>
